...
export default {
  ...
  watch: {
    '$route.query': {
      async  handler (newVal) {
        await this.searchGlobalUser({ limit: 8, page: 1, keyword: this.$route.query.keyword, isLoadMore: false })
      }
    }
  },
  mounted () {
  ...
  async created () {
    await this.searchGlobalUser({ limit: 8, page: 1, keyword: this.$route.query.keyword })
  },
  methods: {
    loadMorePeople () {
      try {
        ...
        this.isDebounce = setTimeout(async () => {
          if (this.$route.path.split('/')[2] === 'people' && this.pageDetailUser.nextPage && !this.isLoadMore) {
            if (!this.isLoadMore && this.pageDetailUser.nextPage && document.documentElement.scrollTop + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 120) {
              ...
              await this.searchGlobalUser({ limit: 8, page: this.pageDetailUser.nextPage, keyword: this.$route.query.keyword, isLoadMore: true })
