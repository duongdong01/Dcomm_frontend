    ...
    <div class=" w-full h-full rounded-lg grid grid-cols-8">
      <div class="col-span-6  rounded-xl">
        <div class="flex h-[60px] items-center   w-full rounded-xl space-x-2  mb-2">
          <div class="flex hover:bg-gray-700 rounded-lg justify-between gap-2 py-2 px-2 text-white font-medium cursor-pointer text-base " :class="$route.path.split('/')[1]==='search' && $route.path.split('/')[2]!=='people' && $route.path.split('/')[2]!=='post' ? 'bg-gray-700': ''" @click="changeRoute('ALL')">
            <div>
              <svg
        ...
        <div v-if="$route.path.split('/')[1]==='search' && $route.path.split('/')[2]!=='people' && $route.path.split('/')[2]!=='post'">
...
export default {
  ...
  mounted () {
    if (this.$route.path.split('/')[1] === 'search' && this.$route.path.split('/')[2] !== 'people' && this.$route.path.split('/')[2] !== 'post') {
  ...
  methods: {
    ...
    changeRoute (type) {
      if (type === 'ALL') {
        this.$router.push({ path: '/search', query: { keyword: this.$route.query.keyword } })
      }
      if (type === 'PEOPLE') {
        this.$router.push({ path: '/search/people', query: { keyword: this.$route.query.keyword } })
      }
      if (type === 'POST') {
        this.$router.push({ path: '/search/post', query: { keyword: this.$route.query.keyword } })
      }
    },
    loadMore () {
      try {
        ...
        this.isDebounce = setTimeout(async () => {
          if (this.$route.path.split('/')[1] === 'search' && this.$route.path.split('/')[2] !== 'people' && this.$route.path.split('/')[2] !== 'post' && this.pageDetail.nextPage && !this.isLoadMore) {
