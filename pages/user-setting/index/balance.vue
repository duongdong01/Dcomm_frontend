<template>
  <div class="flex flex-col space-y-3 balance">
      ...
      <div class="grid grid-cols-2 px-4 text-[16px] font-semibold border-t-[1px] border-gray-700">
        ...
        <div class="flex flex-col py-3 pl-4">
          ...
          <div class="flex w-full justify-center py-1 text-center">
            <a-row class="w-full justify-center flex">
              <a-col :span="12">
                <a-slider v-model="inputWithdraw" :min="1000" :max="Math.floor(token.currentTotal)" :step="1" :disabled="token.currentTotal > 1000 ? false : true" />
              </a-col>
              <a-col :span="4">
                <a-input-number
                  ...
                  :disabled="true"
                  :min="1000"
                  :max="Math.floor(token.currentTotal)"
                  :step="1"
                  style="marginLeft: 16px"
                />
              </a-col>
...
<script>
import Withdraw from '@/components/modal/Withdraw.vue'
export default {
  components: { Withdraw },
  data () {
    return {
  ...
  methods: {
    ...
    async payment () {
      try {
        await this.$api.user.payment({ amount: this.inputValue, description: 'payment' })
      } catch (err) {
        // console.log(err)
      }
    },
