    ...
    <div class="max-w-[500px] w-[500px] bg-gray_850  pb-2 -mt-20 rounded-xl border border-gray-700">
      ...
      <div v-if="isShowSubmitReport">
        ...
        <div v-if="reportSubmit.description.length" class="text-[14px] font-semibold">
          <ul class="px-9">
            <li v-for="(descriptionItem,indexDes) in reportSubmit.description" :key="indexDes" class="list-disc">
              {{ descriptionItem }}
            </li>
          </ul>
        </div>
      ...
      reports: [
        {
          id: 1,
          name: 'Nudity',
          title: "We only remove content that goes against our Community Standards. We don't allow things like:",
          description: [
            'Sexual activity',
        ...
        },
        {
          id: 2,
          name: 'Spam',
          title: 'We don’t allow things like:',
          description: [
        ...
        },
        {
          id: 3,
          name: 'Terrorism',
          title: 'We only remove content that goes against our Community Standards.',
          description: [
      ...
      try {
        ...
        if (report.name === 'Something Else') {
          if (this.reportText.trim().length === 0) {
            ...
            await this.$api.user.reportPost({ postId: this.postId, name: report.name, description: this.reportText })
            this.$toast.success('Report post successfully')
            this.isShowSubmitReport = false
            this.hiddenReportModal()
            this.isCreatePost = false
          }
        } else {
          await this.$api.user.reportPost({ postId: this.postId, name: report.name, description: report.name })
          this.$toast.success('Report post successfully')
          this.isShowSubmitReport = false
          this.hiddenReportModal()
          this.isCreatePost = false
