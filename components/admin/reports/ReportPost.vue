...
export default {
  ...
  async created () {
    await this.listReportPost({ page: 1, limit: 8, sort: 'DESC', keyword: '' })
  },
  methods: {
    showAvatar (avatar) {
      this.avatar = avatar
    ...
    async handleTableChange (pagination, filters, sorter) {
      try {
        ...
        await this.listReportPost({ page: pagination.current, limit: 8, sort: 'DESC', keyword: this.searchText })
      } catch (error) {
      }
    },
    async listReportPost ({ page, limit, sort, keyword }) {
      try {
        const reportData = await this.$api.admin.listReportPost({ page, limit, sort, keyword })
